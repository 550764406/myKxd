<template>
	<div id="Address">
	 <svg style="position: absolute; width: 0; height: 0; overflow: hidden;" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        <defs><!--图标-->
            <symbol id="icon-add" viewBox="0 0 32 32">
                <title>add2</title>
                <path class="path1" d="M15 17h-13.664c-0.554 0-1.002-0.446-1.002-1 0-0.552 0.452-1 1.002-1h13.664v-13.664c0-0.554 0.446-1.002 1-1.002 0.552 0 1 0.452 1 1.002v13.664h13.664c0.554 0 1.002 0.446 1.002 1 0 0.552-0.452 1-1.002 1h-13.664v13.664c0 0.554-0.446 1.002-1 1.002-0.552 0-1-0.452-1-1.002v-13.664z"></path>
            </symbol>
            <symbol id="icon-ok" viewBox="0 0 39 32">
                <title>ok</title>
                <path class="path1" d="M14.084 20.656l-7.845-9.282c-1.288-1.482-3.534-1.639-5.016-0.351s-1.639 3.534-0.351 5.016l10.697 12.306c1.451 1.669 4.057 1.623 5.448-0.096l18.168-22.456c1.235-1.527 0.999-3.765-0.528-5.001s-3.765-0.999-5.001 0.528l-15.573 19.337z"></path>
            </symbol>
            <symbol id="icon-edit" viewBox="0 0 32 32">
                <title>edit</title>
                <path class="path1" d="M25.599 11.292l-4.892-4.892 3.825-3.825 4.892 4.892-3.825 3.825zM4.732 23.308l3.959 3.959-5.939 1.98 1.98-5.939zM10.666 26.225l-4.892-4.892 13.425-13.425 4.892 4.892-13.425 13.425zM31.687 6.713l-6.4-6.4c-0.417-0.417-1.091-0.417-1.508 0l-20.267 20.267c-0.114 0.115-0.191 0.25-0.242 0.393-0.003 0.009-0.012 0.015-0.015 0.025l-3.2 9.6c-0.128 0.383-0.029 0.806 0.257 1.091 0.203 0.204 0.476 0.313 0.754 0.313 0.112 0 0.227-0.017 0.337-0.054l9.6-3.2c0.011-0.003 0.017-0.013 0.027-0.016 0.142-0.052 0.276-0.128 0.39-0.242l20.267-20.267c0.417-0.416 0.417-1.091 0-1.508v0z"></path>
            </symbol>
            <symbol id="icon-del" viewBox="0 0 26 32">
                <title>delete</title>
                <path class="path1" d="M17.723 28c0.543 0 0.984-0.448 0.984-1v-12c0-0.552-0.441-1-0.984-1s-0.985 0.448-0.985 1v12c0 0.552 0.441 1 0.985 1v0zM7.877 28c0.543 0 0.984-0.448 0.984-1v-12c0-0.552-0.441-1-0.984-1s-0.985 0.448-0.985 1v12c0 0.552 0.441 1 0.985 1v0zM12.8 28c0.543 0 0.985-0.448 0.985-1v-12c0-0.552-0.441-1-0.985-1s-0.984 0.448-0.984 1v12c0 0.552 0.441 1 0.984 1v0zM23.631 4h-5.908v-2c0-1.104-0.882-2-1.969-2h-5.908c-1.087 0-1.969 0.896-1.969 2v2h-5.908c-1.087 0-1.969 0.896-1.969 2v2c0 1.104 0.882 2 1.969 2v18c0 2.208 1.765 4 3.939 4h13.784c2.174 0 3.938-1.792 3.938-4v-18c1.087 0 1.969-0.896 1.969-2v-2c0-1.104-0.882-2-1.969-2v0zM9.846 3c0-0.552 0.441-1 0.984-1h3.938c0.544 0 0.985 0.448 0.985 1v1h-5.908v-1zM21.662 28c0 1.104-0.882 2-1.969 2h-13.784c-1.087 0-1.97-0.896-1.97-2v-18h17.723v18zM22.646 8h-19.692c-0.543 0-0.985-0.448-0.985-1s0.441-1 0.985-1h19.692c0.543 0 0.984 0.448 0.984 1s-0.441 1-0.984 1v0z"></path>
            </symbol>
            <symbol id="icon-clock" viewBox="0 0 32 32">
                <title>clock</title>
                <path class="path1" d="M29.333 16c0-7.364-5.97-13.333-13.333-13.333s-13.333 5.97-13.333 13.333c0 7.364 5.97 13.333 13.333 13.333s13.333-5.97 13.333-13.333v0 0 0 0 0 0zM0 16c0-8.837 7.163-16 16-16s16 7.163 16 16c0 8.837-7.163 16-16 16s-16-7.163-16-16zM14.667 14.667v1.333h2.667v-10.667h-2.667v9.333zM24 18.667h1.333v-2.667h-10.667v2.667h9.333z"></path>
            </symbol>
        </defs>
    </svg>
	<div class="check-step">
		<ul>
			<li class="cur">地址确认</li>
            <li>查看订单</li>
            <li>支付</li>
            <li>订单确认</li>
		</ul>
	</div>

	<div class="checkout-title">
        <span>配送地址</span>
    </div>

    <div class="addr-list-wrap">
        <div class="addr-list">
            <ul>
            	<li v-for="(item,index) in filterAddressList" :class="{'check': item.isDefault}">
				   <dl>
					    <dt>{{item.userName}}</dt> 
					    <dd class="address">{{item.streetName}}</dd> 
					    <dd class="tel">{{item.postCode}}</dd>
				   </dl> 
				   <div class="addr-opration addr-edit">
				    <a href="javascript:;" class="addr-edit-btn">
				     <svg class="icon icon-edit">
				      <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-edit"></use>
				     </svg></a>
				   </div> 
				   <div class="addr-opration addr-del">
				    <a href="javascript:;" class="addr-del-btn" @click="deleteAddress(item,index)">
				     <svg class="icon icon-del">
				      <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-del"></use>
				     </svg></a>
				   </div> 
				   <div class="addr-opration addr-set-default" v-show="!item.isDefault">
				    <a href="javascript:;" class="addr-set-default-btn" @click="setDefaultAddress(item)"><i>设为默认</i></a>
				   </div> 
				   <div class="addr-opration addr-default" v-show="item.isDefault">
				    默认地址
				   </div>
				</li>
				
				<li class="addr-new">
				   <div class="add-new-inner">
				    <i class="icon-add">
				     <svg class="icon icon-add">
				      <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-add"></use>
				     </svg></i> 
				    <p>添加新地址</p>
				   </div>
				</li>
            </ul>
        </div>

        <div class="shipping-addr-more">
        	<a href="javascript:;" class="addr-more-btn up-down-btn" @click="toggleMore" v-text="moreText"><i class="i-up-down"><i class="i-up-down-l"></i> <i class="i-up-down-r"></i></i></a>
        </div>
    </div>

    <div class="checkout-title">
        <span>配送方式</span>
    </div>

     <div class="shipping-method-wrap">
        <div class="shipping-method">
            <ul>
			   <li :class='{"check": shippingMethod == 1}' @click="shippingMethod =1">
				   <div class="name">标准配送</div> 
				   <div class="price">Free</div>
			   </li> 
			   <li @click="shippingMethod = 2" :class='{"check": shippingMethod ==2}'>
				   <div class="name">高级配送</div> 
				   <div class="price">180</div>
				</li>
			  </ul>
        </div>
     </div>

     <div class="next-btn-wrap"><a href="javascript:;" class="btn btn--red">下一步</a></div>

</div>
</template>

<script>
import Vue from 'vue';
import axios from "axios";
export default{
	data(){
		return {
			addressList: [],
			limitNum: 2,
			moreText: "more",
			shippingMethod: 1
		}
	},
	methods:{
		toggleMore(){
			this.limitNum = (this.limitNum == this.addressList.length?2: this.addressList.length);
			this.moreText = (this.moreText == "more"?"hide": "more");
		},
		setDefaultAddress(item){
			this.addressList.forEach((value)=>{
		        value.isDefault=false;
		      })
		      item.isDefault=true;
		},
		deleteAddress(item,index){
			this.addressList.splice(index,1)
		}
	},
	mounted(){
		this.$nextTick(function(){
			axios.get("http://localhost:8085/src/data/addressData.json").then((res)=>{
				if(res.status == 200){
					this.addressList = res.data.result;
				}
			}).catch(function(errorObj){
				console.log(errorObj);
			})
		})
	},
	computed:{
		filterAddressList(){
			return this.addressList.slice(0,this.limitNum);
		}
	}
}
</script>

<style rel="stylesheet/scss" lang="scss" scoped>
/*** checkbox *****/
input[type="checkbox"]{
    display: none;
}
input[type="checkbox"]+label {
    display: inline-block;
    width: 58%;
    margin-left: 5px;
    text-align: left;
    -webkit-box-sizing: border-box;
}

label::before {
    content: "";
    display: inline-block;
    width: 20px;
    height: 20px;
    background: #fff;
    border: 1px solid #ccc;
    margin-top: 5px;
    vertical-align: middle;
    -webkit-border-radius: 50%;
    margin-right: 5px;
    -webkit-box-sizing:border-box;
  -webkit-transition:background ease-in .5s
}

input[type="checkbox"]:checked+label::before{
    background-color: #fff;
    border: 5px solid rgb(53, 183, 111);
}

#Address{
	background: #fff;
	padding: 10px 30px;
}
.check-step{
	padding: 0 5px 0 5px;
	ul{
		display: -webkit-flex;
		display: flex;
		margin: 25px 0 50px 0;
		&:after {
	      visibility: hidden;
	      display: block;
	      content: "";
	      clear: both; 
	 	}
	 	li{
	 		position: relative;
		    float: left;
		    -ms-flex: 1;
		    flex: 1;
		    width: 25%;
		    line-height: 1.5em;
		    padding: 0 1em 1em 1em;
		    border-bottom: 2px solid #ccc;
		    color: #999;
		    font-size: 15px;
		    text-align: center;
			    &:after {
		      		position: absolute;
				    bottom: -7px;
				    left: 50%;
				    margin-left: -7px;
				    content: "";
				    width: 14px;
				    height: 14px;
				    border-radius: 50%;
				    background: #ccc;
				}
				&.cur{
				      border-color: #79aeff;
				      color: #79aeff; 
				      font-weight: bold;
				      &:after {
		     				 background: #79aeff; 
		     			}
		 			}
	 		}
	}
}

.checkout-title{
	margin: 0 5px 0 5px;
	position: relative;
	margin-bottom: 41px;
	text-align: center;
	span{
		position: relative;
		padding: 0 1em;
		background: #fff;
		font-size: 20px;
		font-weight: bold;;
		color: #605f5f;
		z-index: 1;
	}
	&:before{
		position: absolute;
		top: 50%;
		left: 0;
		content: "";
		width: 100%;
		height: 1px;
		background: #ccc;
		z-index: 0
	};
}
.addr-list{
	ul{
		&:after{
			visibility: hidden;
			display: block;
			content: " ";
			clear: both;
		};
		li{
			position: relative;
			float: left;
			width: 23.5%;
			height: 162px;
			margin: 10px 2% 40px 20px;
			background: #fff;
			border: 2px solid #e9e9e9;
			overflow: hidden;
			cursor: pointer;
			dt{
				font-size: 18px;
				padding: 10px 0 0 20px;
			}
			dd{
				line-height: 20px;
				padding: 10px 0 0 20px;
			}
			.address{
				height: 40px;
				overflow: hidden;
			}
			.tel{
				color: #605F5F;
			}
			.addr-opration{
				position: absolute;
				.icon{
					width: 100%;
					height: 100%;
				}
			}
			.addr-opration:hover .icon{
				fill: #EE7A23;
			}
			.addr-edit{
				display: none;
				top: 20px;
				right: 20px;
				width: 20px;
				height: 20px;
			}
			.addr-del{
				display: none;
				bottom: 20px;
				right: 20px;
				width: 20px;
				height: 20px;
				&:hover{
					display: block;
				};
			}

			&:hover{
				.addr-edit{
					display: block;
				}
				.addr-del{
					display: block;
				}
				.addr-set-default {
				  	display: block;
				}
			};
			.addr-set-default{
				bottom: 20px;
				left: 20px;
				color: #79aeff
			}
			.addr-default{
				bottom: 20px;
				left: 20px;
				color: #79aeff
			}
			.addr-set-default {
			  	display: none;
			}
			&:nth-child(4n) {
    			margin-right: 0; 
    		}
    		&:hover{
    			border-color: #79aeff;
    		};
    		&.addr-new {
			    color: #333;
			    text-align: center; 
			    .add-new-inner {
      				padding-top: 20px; 
      				.icon-add {
				        display: inline-block;
				        width: 50px;
				        height: 50px; 
				        .icon-add {
					          width: 50px;
					          height: 50px;
					          fill: #605F5F; 
					     }
				    }
				    p{
				    	 margin-top: 10px;
				    }
  				}
			}

			&.check {
				border-color: #79aeff;
				border-width: 2px; 
				.addr-opration {
      				display: block; 
  				}
			}
		}
	}
}
.shipping-addr-more {
  clear: both;
  text-align: center;
  color: #79aeff;
  font-weight: bold;
  margin-bottom: 40px; 
  a{
  	color: #79aeff;
  }
}

.shipping-method {
  text-align: center; 
  li {
	    display: inline-block;
	    width: 300px;
	    margin: 5px;
	    padding: 10px;
	    background: #fff;
	    border: 2px solid #e9e9e9;
	    font-size: 18px;
	    line-height: 26px;
	    text-align: center;
	    color: #999;
	    font-family: "Moderat";
	    cursor: pointer; 
	    &.check {
		      border-color: #79aeff;
		      color: #333; 
  		}
  		&:hover {
      			border-color: #79aeff; 
      	}
	    .price {
      			font-weight: bold; 
      	}
	}
}
.next-btn-wrap {
	width: 100%;
	padding: 22px 0;
  	border-top: 1px solid #e9e9e9;
  	text-align: center; 
  	margin-top: 40px;

}
</style>